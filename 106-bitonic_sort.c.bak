 ...
 */
void bitonic_merge(int *array, size_t size, size_t seq, int flow)
{
	...
	if (seq && seq > 1)
	{
		...
		bitonic_merge(array +jump, size, jump, flow);
	}
}
...
void bitonic_seq(int *array, size_t size, size_t seq, int flow)
{
	...
	if (seq && seq > 1)
	{
		...
		print_array(array, seq);

		bitonic_seq(array, size, cut, 1);
		bitonic_seq(array + cut, size,cut, 0);
